...
def normal_reservation(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Reserva creada exitosamente para el vehículo {reservation.vehicle.license_plate}.'
            )
...
def quick_reservation(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Reserva rápida creada exitosamente para {reservation.customer_name} - Vehículo {reservation.vehicle_plate}.'
            )
            return redirect('parking:reservation_list')
        else:
            messages.error(request, 'Por favor corrija los errores del formulario.')
    ...
    context = {
        ...
        'submit_text': 'Crear Reserva Rápida',
